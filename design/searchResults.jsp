	...
	<div class="offcanvas-body">
					...
					<c:forEach var="countEntry" items="${fieldData.valueCountMap}">
						<c:if test="${countEntry.value != 0 && fe:labelexists(countEntry.key)}">
							<li class="list-group-item"><la:link
									href="/search?q=${f:u(q)}&ex_q=label%3a${f:u(countEntry.key)}&sdh=${f:u(fe:sdh(sdh))}${fe:pagingQuery(null)}${fe:facetQuery()}${fe:geoQuery()}">
		...
		<c:if test="${!empty ex_q}">
			<div class="d-flex justify-content-end">
				<la:link href="/search?q=${f:u(q)}" styleClass="btn btn-link btn-sm">
